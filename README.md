...
Текущая составная архитектура работает одним проходом при 48 кГц:

`Q4 + Sustain + Q3 (3x3) -> Q2 scalar -> Tone + Q1 (2x2)`.

На полном четырёхсекундном аккорде при размере DMA-полубуфера 32 получено
`0/6000` deadline overruns: максимум 107126 тактов при бюджете 113312.
Настоящий ADC/DMA/DSP/DAC runtime также работает без overruns; до установки
внешнего входного смещения он запускается в безопасном режиме цифрового нуля.
...
```powershell
platformio run -e composed_block_benchmark
